...
export function expectedAndroidVersion(appVersion: string, commitsAhead?: number, buildDate?: string) {
  if (appVersion.includes("-whispercode-")) return appVersion

  const commits = commitsAhead ?? 0
  const date = buildDate ?? new Date().toISOString().slice(0, 10).replaceAll("-", "")
  return `${appVersion}-whispercode-${commits}-${date}`
}
...
async function main() {
  try {
    ...
    const expectedVersion = expectedAndroidVersion(appVersion, commitsAhead)

    if (androidVersion !== expectedVersion) {
      console.log(`Syncing Android version: ${androidVersion} -> ${expectedVersion}`)
      androidPkg.version = expectedVersion
      ...
      try {
        ...
        execSync(`git commit -m "chore: sync android version to ${expectedVersion}"`, { stdio: "inherit", env })
        console.log(`✅ Automatically committed version sync to ${expectedVersion}`)
      } catch (e) {
