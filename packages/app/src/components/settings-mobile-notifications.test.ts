...
describe("settings mobile notifications", () => {
  ...
  test("shows mobile push settings only on iOS", () => {
    expect(mobilePushSettingsVisible("ios")).toBe(true)
    expect(mobilePushSettingsVisible("android")).toBe(false)
    expect(mobilePushSettingsVisible("web")).toBe(false)
    expect(mobilePushSettingsVisible("desktop")).toBe(false)
  })
  ...
  test("does not expose Android host or direct delivery code", async () => {
    const file = Bun.file(new URL("./settings-mobile-notifications.tsx", import.meta.url))
    const text = await file.text()
    expect(text).not.toContain("push-host")
    expect(text).not.toContain("AndroidHostDeliveryView")
    expect(text).not.toContain("settings-push-host")
    expect(text).toContain("MobilePushSettingsGate")
  })
})
