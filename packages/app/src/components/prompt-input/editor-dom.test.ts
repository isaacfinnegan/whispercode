...
describe("prompt-input editor dom", () => {
  ...
  test.each([
    ["data-mention", "file", "@My File.ts"],
    ["data-mention", "agent", "@code reviewer"],
    ["data-mention", "reference", "@Shared Docs"],
    ["data-type", "file", "@My File.ts"],
    ["data-type", "agent", "@code reviewer"],
  ] as const)("deletes an atomic %s=%s token containing whitespace", (attribute, kind, content) => {
    const container = document.createElement("div")
    const pill = document.createElement("span")
    pill.setAttribute(attribute, kind)
    pill.contentEditable = "false"
  ...
  test.each(["div", "p"] as const)("deletes through a nested %s block wrapper", (tag) => {
    const container = document.createElement("div")
    ...
    const text = getEditorText(container)
    const span = getDeleteWordRange(text, getSelectionRange(container))
    const range = document.createRange()
    setSelectionRange(container, range, span!.start, span!.end)
    range.deleteContents()
    setCursorPosition(container, span!.start)
    ...
    expect(block.textContent).toBe("alpha")
